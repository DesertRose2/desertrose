...
/obj/item/bombcore/training
	name = "dummy payload"
	desc = "A replica of a bomb payload, used for training. It's not intended to explode but to announce that it WOULD have exploded, then rewire itself to allow for more training."
	var/defusals = 0
	var/attempts = 0
...
/obj/item/bombcore/chemical/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/crowbar) && beakers.len > 0)
		...
		for (var/obj/item/B in beakers)
			...
			beakers -= B
		return
	else if(istype(I, /obj/item/reagent_containers/glass/beaker/bomb))
...
/obj/item/bombcore/chemical/CheckParts(list/parts_list)
	...
	for(var/obj/item/grenade/chem_grenade/G in src)
		...
		for(var/obj/item/reagent_containers/glass/beaker/bomb/B in G)
			if(beakers.len < max_beakers)
				beakers += B
				B.forceMove(src)
			else
