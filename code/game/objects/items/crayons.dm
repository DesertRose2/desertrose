...
/obj/item/toy/crayon/proc/draw_on(atom/target, mob/user, proximity, params)
	...
	if(!instant)
		...
		to_chat(user, SPAN_NOTICE("You spray a [temp] on \the [target]."))
...
/obj/item/toy/crayon/spraycan/draw_on(atom/target, mob/user, proximity, params)
	...
	if(iscarbon(target))
		...
		if(C != user)
			C.visible_message(SPAN_DANGER("[user] sprays [src] into the face of [C]!"), "<span class='userdanger'>[user] sprays [src] into your face!</span>")
