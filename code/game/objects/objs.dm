	...
	if(href_list[VV_HK_ARMOR_MOD])
		...
		if (islist(result))
			if (result["button"] != 2) // If the user pressed the cancel button
				...
				message_admins("<span class='notice'>[key_name_admin(usr)] modified the armor on [src] ([type]) to melee: [armor.melee], bullet: [armor.bullet], laser: [armor.laser], bomb: [armor.bomb], bio: [armor.bio], rad: [armor.rad], acid: [armor.acid]")
