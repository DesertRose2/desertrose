...
/obj/machinery/disposal/proc/stuff_mob_in(mob/living/target, mob/living/user)
	...
	if(user == target)
		...
		target.visible_message(SPAN_DANGER("[user] starts putting [target] into [src]."), "<span class='userdanger'>[user] starts putting you into [src]!</span>")
	if(do_mob(user, target, 20))
		...
		if(user == target)
			...
			target.visible_message(SPAN_DANGER("[user] has placed [target] in [src]."), "<span class='userdanger'>[user] has placed [target] in [src].</span>")
