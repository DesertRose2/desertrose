...
/obj/item/clothing/proc/disable_zone(def_zone, damage_type)
	...
	if(iscarbon(loc))
		...
		C.visible_message(SPAN_DANGER("The [zone_name] on [C]'s [src.name] is [break_verb] away!"), "<span class='userdanger'>The [zone_name] on your [src.name] is [break_verb] away!</span>", vision_distance = COMBAT_MESSAGE_RANGE)
