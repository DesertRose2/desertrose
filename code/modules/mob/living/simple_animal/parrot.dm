		...
		if(Adjacent(parrot_interest))
			...
			if(isliving(parrot_interest))
				...
				if(!parrot_perch || parrot_interest.loc != parrot_perch.loc)
					...
					visible_message("[src] grabs [held_item]!", SPAN_NOTICE("You grab [held_item]!"), "<span class='italics'>You hear the sounds of wings flapping furiously.</span>")
...
/mob/living/simple_animal/parrot/proc/steal_from_mob()
	...
	for(var/mob/living/carbon/C in view(1,src))
		...
		if(stolen_item)
			...
			visible_message("[src] grabs [held_item] out of [C]'s hand!", SPAN_NOTICE("You snag [held_item] out of [C]'s hand!"), "<span class='italics'>You hear the sounds of wings flapping furiously.</span>")
			return held_item
...
/mob/living/simple_animal/parrot/Poly/say(message, bubble_type,list/spans = list(), sanitize = TRUE, datum/language/language = null, ignore_spam = FALSE, forced = null)
	. = ..()
...
/mob/living/simple_animal/parrot/Poly/ghost/proc/Possess(mob/living/carbon/human/H)
	...
	parrot_interest = null
	H.visible_message(SPAN_DANGER("[src] dive bombs into [H]'s chest and vanishes!"), "<span class='userdanger'>[src] dive bombs into your chest, vanishing! This can't be good!</span>")
