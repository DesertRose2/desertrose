	...
	switch(soft)
		...
		if("doorjack")
			...
			if(href_list["cable"])
				...
				T.visible_message(SPAN_WARNING("A port on [src] opens to reveal [cable], which promptly falls to the floor."), "<span class='italics'>You hear the soft click of something light and hard falling to the ground.</span>")
		if("loudness")
