			...
			if(istype(W, /obj/item/wrench))
				...
				if (W.use_tool(src, user, 30, volume=50))
					...
					user.visible_message("[user.name] deconstructs [src].", \
						SPAN_NOTICE("You deconstruct [src]."), "<span class='italics'>You hear a ratchet.</span>")
		...
		if(2)
			...
			if(istype(W, /obj/item/wirecutters))
				...
				user.visible_message("[user.name] removes the wiring from [src].", \
					SPAN_NOTICE("You remove the wiring from [src]."), "<span class='italics'>You hear clicking.</span>")
				W.play_tool_sound(src, 100)
				return

			if(istype(W, /obj/item/screwdriver))
				user.visible_message("[user.name] closes [src]'s casing.", \
					SPAN_NOTICE("You close [src]'s casing."), "<span class='italics'>You hear screwing.</span>")
	...
	// attempt to stick weapon into light socket
		...
		if(istype(W, /obj/item/screwdriver)) //If it's a screwdriver open it.
			...
			user.visible_message("[user.name] opens [src]'s casing.", \
				SPAN_NOTICE("You open [src]'s casing."), "<span class='italics'>You hear a noise.</span>")
			deconstruct()
		else
...
/obj/item/light/proc/shatter()
	if(status == LIGHT_OK || status == LIGHT_BURNED)
		visible_message(SPAN_DANGER("[src] shatters."),"<span class='italics'>You hear a small glass object shatter.</span>")
		status = LIGHT_BROKEN
		force = 5
		playsound(src.loc, 'sound/effects/glasshit.ogg', 75, 1)
		update()
...
/obj/machinery/light/proc/flicker(amount = rand(10, 20), spark = TRUE, sounds = TRUE, loud = TRUE)
	...
	if(on && status == LIGHT_OK)
		if(loud)
			visible_message(SPAN_WARNING("[src] begins flickering!"),"<span class='italics'>You hear an electrical sparking.</span>")
