...
/obj/item/grenade/homemade/proc/primefuse(mob/user, delayoverride, msg = TRUE, volume = 60) 
...
/obj/item/grenade/homemade/proc/primetimer(mob/user, delayoverride, msg = TRUE, volume = 60) 
	var/turf/T = get_turf(src)
	log_grenade(user, T) //Inbuilt admin procs already handle null users
	if(user)
		add_fingerprint(user)
