...
/obj/item/dronespeak_manual/attack(mob/living/M, mob/living/user)
	...
	if(isdrone(M) || issilicon(M))
		if(M.has_language(/datum/language/drone))
			M.visible_message(SPAN_DANGER("[user] beats [M] over the head with [src]!"), "<span class='userdanger'>[user] beats you over the head with [src]!</span>", "<span class='hear'>You hear smacking.</span>")
		else
			M.visible_message(SPAN_NOTICE("[user] teaches [M] by beating [M.p_them()] over the head with [src]!"), "<span class='boldnotice'>As [user] hits you with [src], chitters resonate in your mind.</span>", "<span class='hear'>You hear smacking.</span>")
