...
/obj/item/gun/ballistic/revolver/detective/screwdriver_act(mob/living/user, obj/item/I)
	...
	if("38" in magazine.caliber)
		...
		if(magazine.ammo_count())
			...
			user.visible_message(SPAN_DANGER("[src] goes off!"), "<span class='userdanger'>[src] goes off in your face!</span>")
			return TRUE
		...
		if(chambered)
			...
			if(AC.fire_casing(user, user))
				...
				if(zone == BODY_ZONE_HEAD || zone == BODY_ZONE_PRECISE_EYES || zone == BODY_ZONE_PRECISE_MOUTH)
					...
					user.visible_message(SPAN_DANGER("[user.name] cowardly fires [src] at [user.p_their()] [affecting.name]!"), "<span class='userdanger'>You cowardly fire [src] at your [affecting.name]!</span>", "<span class='italics'>You hear a gunshot!</span>")
				chambered = null
				return
...
/obj/item/gun/ballistic/revolver/reverse/can_trigger_gun(mob/living/user)
	...
	if(process_fire(user, user, FALSE, null, BODY_ZONE_HEAD))
		user.visible_message(SPAN_WARNING("[user] somehow manages to shoot [user.p_them()]self in the face!"), "<span class='userdanger'>You somehow shoot yourself in the face! How the hell?!</span>")
		user.emote("scream")
