...
/obj/item/assembly/flash/proc/burn_out() //Made so you can override it if you want to have an invincible flash from R&D or something.
	...
	if(ismob(loc))
		var/mob/M = loc
		M.visible_message(SPAN_DANGER("[src] burns out!"),"<span class='userdanger'>[src] burns out!</span>")
	else
...
/obj/item/assembly/flash/attack(mob/living/M, mob/user)
	...
	if(iscarbon(M))
		...
		user.visible_message("<span class='disarm'>[user] overloads [R]'s sensors with the flash!</span>", SPAN_DANGER("You overload [R]'s sensors with the flash!"))
		return TRUE

	user.visible_message("<span class='disarm'>[user] fails to blind [M] with the flash!</span>", SPAN_WARNING("You fail to blind [M] with the flash!"))
	...
	if(targeted)
		if(M.flash_act(1, 1))
			...
			if(M.hypnosis_vulnerable())
				hypnosis = TRUE
			if(user)
				user.visible_message("<span class='disarm'>[user] blinds [M] with the flash!</span>", SPAN_DANGER("You hypno-flash [M]!"))
			...
			user.visible_message("<span class='disarm'>[user] fails to blind [M] with the flash!</span>", SPAN_WARNING("You fail to hypno-flash [M]!"))
		else
			to_chat(M, SPAN_DANGER("[src] fails to blind you!"))
