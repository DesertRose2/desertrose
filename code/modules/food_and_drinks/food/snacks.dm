	...
	if(iscarbon(M))
		...
		if(M == user)								//If you're eating it yourself.
			if(junkiness && M.satiety < -150 && M.nutrition > NUTRITION_LEVEL_STARVING + 50 )
				...
				user.visible_message(SPAN_NOTICE("[user] hungrily takes a [eatverb] from \the [src], gobbling it down!"), SPAN_NOTICE("You hungrily take a [eatverb] from \the [src], gobbling it down!"))
			else if(fullness > 50 && fullness < 150)
				user.visible_message(SPAN_NOTICE("[user] hungrily takes a [eatverb] from \the [src]."), SPAN_NOTICE("You hungrily take a [eatverb] from \the [src]."))
			else if(fullness > 150 && fullness < 500)
				user.visible_message(SPAN_NOTICE("[user] takes a [eatverb] from \the [src]."), SPAN_NOTICE("You take a [eatverb] from \the [src]."))
			else if(fullness > 500 && fullness < 600)
				user.visible_message(SPAN_NOTICE("[user] unwillingly takes a [eatverb] of a bit of \the [src]."), SPAN_WARNING("You unwillingly take a [eatverb] of a bit of \the [src]."))
			else if(fullness > (600 * (1 + M.overeatduration / 2000)))	// The more you eat - the more you can eat
				user.visible_message(SPAN_WARNING("[user] cannot force any more of \the [src] to go down [user.p_their()] throat!"), SPAN_DANGER("You cannot force any more of \the [src] to go down your throat!"))
				return 0
		else
