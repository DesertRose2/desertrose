...
/datum/martial_art/the_sleeping_carp/proc/strongPunch(mob/living/carbon/human/A, mob/living/carbon/human/D)
	...
	if(prob(10))
		...
		D.visible_message(SPAN_WARNING("[D] staggers as the blow strikes them with inhuman force!"), "<span class='userdanger'>You are struck with incredible precision by [A]!</span>")
...
/datum/martial_art/the_sleeping_carp/on_projectile_hit(mob/living/carbon/human/A, obj/item/projectile/P, def_zone)
	...
	if(!isturf(A.loc)) //NO MOTHERFLIPPIN MECHS!
		return BULLET_ACT_HIT
	if(A.in_throw_mode)
		A.visible_message(SPAN_DANGER("[A] effortlessly swats the projectile aside! They can deflect projectile with their bare hands!"), "<span class='userdanger'>You deflect the projectile!</span>")
