...
/datum/wound/blunt/moderate/crush()
	if(prob(33))
		victim.visible_message(SPAN_DANGER("[victim]'s dislocated [limb.name] pops back into place!"), "<span class='userdanger'>Your dislocated [limb.name] pops back into place! Ow!</span>")
		remove_wound()
...
/datum/wound/blunt/moderate/treat(obj/item/I, mob/user)
	...
	if(victim == user)
		...
		victim.visible_message(SPAN_DANGER("[user] finishes resetting [victim.p_their()] [limb.name]!"), "<span class='userdanger'>You reset your [limb.name]!</span>")
	else
		limb.receive_damage(brute=10, wound_bonus=CANT_WOUND)
		user.visible_message(SPAN_DANGER("[user] finishes resetting [victim]'s [limb.name]!"), "<span class='nicegreen'>You finish resetting [victim]'s [limb.name]!</span>", victim)
...
/datum/wound/blunt/severe
	...
	treatable_by = list(/obj/item/stack/sticky_tape/surgical, /obj/item/stack/medical/bone_gel, /obj/item/stack/medical/bone_gel/hydra)
...
/datum/wound/blunt/proc/tape(obj/item/stack/sticky_tape/surgical/I, mob/user)
	...
	if(user != victim)
		...
		victim.visible_message(SPAN_NOTICE("[victim] finishes applying [I] to [victim.p_their()] [limb.name], !"), "<span class='green'>You finish applying [I] to your [limb.name], and you immediately begin to feel your bones start to reform!</span>")
